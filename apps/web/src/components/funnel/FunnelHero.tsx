...
export function FunnelHero({
  ...
  heroImgDesktop = '/images/hero-portrait.png',
  heroImgMobile = '/images/hero-portrait-mobile.png',
  heroImgAlt = 'A person smiling confidently',
  navCta,
}: {
  logoSrc?: string;
  ...
  heroImgAlt?: string;
  /* when set, the nav shows a "Book A Consultation"-style CTA instead of the
     "Save over 85%…" text (used on /funnel-2026). `labelShort` is used on the
     narrow mobile header. */
  navCta?: { label?: string; labelShort?: string; href?: string };
} = {}) {
  return (
    <div className="hero-unit">
      ...
      <nav className="f-nav" aria-label="Primary">
        ...
        />
        {navCta ? (
          <a className="f-nav__cta" href={navCta.href ?? BOOKING_PATH}>
            <span className="f-nav__cta-full">{navCta.label ?? 'Book A Consultation'}</span>
            <span className="f-nav__cta-short">{navCta.labelShort ?? 'Consultation'}</span>
          </a>
        ) : (
          <span className="f-nav__save">
            <svg
              className="f-nav__save-ico"
              width="20"
              height="18"
              viewBox="0 0 22 20"
              fill="currentColor"
            ...
            Save over 85% on your first month
          </span>
        )}
      </nav>
